...
const PaymentOverview = () => {
    ...
    const data = useAllPayroll?.overviewData?.map((item: DataType, index: number) => ({
        key: index + 1,
    ...
    const columns: ColumnsType<DataType> = [
        ...
        },
        {
            title: "Action",
            key: "action",
            render: (_, record) => <button className="bg-primary text-white px-4 py-1 rounded " onClick={() => navigate(`/payment-reports?instructorId=${record?.instructorId}`)}>View Details</button>,
        },
    ]
